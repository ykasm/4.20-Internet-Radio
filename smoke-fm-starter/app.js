...
let player=null,ready=false,pending=false,muted=false,timer=null,station="SMOKE FM";
...
function signal(s){$("signalText")&&($("signalText").textContent=s)}
function setPlaying(v){$("playBtn").textContent=v?"Ⅱ":"▶";$("visualizer").classList.toggle("playing",v)}
function updateTrack(){
 if(!ready)return;
 const d=player.getVideoData?player.getVideoData():{}, title=d.title||"4.20 INTERNET RADIO";
 $("trackTitle").textContent=title;$("sideTrack").textContent=title;$("trackArtist").textContent="4.20 INTERNET RADIO — YOUTUBE";$("sideArtist").textContent="LIVE FROM YOUTUBE";
 $("stationLabel").textContent=station;document.title=`${title} — SMOKE FM`;
 if(d.video_id){$("albumArt").style.backgroundImage=`linear-gradient(rgba(3,5,3,.05),rgba(3,5,3,.38)),url("https://i.ytimg.com/vi/${d.video_id}/hqdefault.jpg")`;$("albumArt").style.backgroundSize="cover";$("albumArt").style.backgroundPosition="center"}
}
function updateProgress(){
 if(!ready)return;
 const d=player.getDuration(),c=player.getCurrentTime(),p=d?Math.min(100,c/d*100):0;
 $("progressFill").style.width=p+"%";$("progressHit").querySelector("i").style.left=p+"%";$("currentTime").textContent=fmt(c);$("duration").textContent=fmt(d)
}
...
const source=document.createElement("div");source.className="youtube-source";source.innerHTML='<div id="youtube-player"></div>';document.body.appendChild(source);
const yt=document.createElement("script");yt.src="https://www.youtube.com/iframe_api";document.head.appendChild(yt);
$("playBtn").onclick=toggle;$("nextBtn").onclick=next;$("prevBtn").onclick=prev;$("shuffleBtn").onclick=shuffle;
$("muteBtn").onclick=()=>{if(!ready)return;muted=!muted;muted?player.mute():player.unMute();$("muteBtn").textContent=muted?"×":"♫"};
$("volume").oninput=e=>{if(!ready)return;const v=Number(e.target.value);if(v===0){player.mute();muted=true;$("muteBtn").textContent="×"}else{player.unMute();player.setVolume(v);muted=false;$("muteBtn").textContent="♫"}};
$("progressHit").onclick=e=>{if(!ready)return;const r=e.currentTarget.getBoundingClientRect();player.seekTo(player.getDuration()*Math.max(0,Math.min(1,(e.clientX-r.left)/r.width)),true)};
...
setInterval(()=>{const n=1600+Math.floor(Math.random()*180);$("listeners").textContent=n.toLocaleString();$("listenersTop").textContent=n.toLocaleString()},5000);
